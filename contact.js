function menuc(){

    document.getElementById("ul1").style.display = "none";
    
    document.getElementById("mirror").style.display = "none";
    
    document.getElementById("menuc").style.display = "none";
    
    document.getElementById("menuo").style.display = "block";

    document.getElementById("contactcall").style.display = "block";

        document.getElementById("contactadd").style.display = "block";

        document.getElementById("contactmail").style.display = "block";

        document.getElementById("social").style.display = "block";

        document.getElementById("mirror1").style.display = "block";
    
    
    }
    
    function menuo(){
    
        document.getElementById("ul1").style.display = "block";
        
        document.getElementById("mirror").style.display = "block";
        
        document.getElementById("menuc").style.display = "block";
        
        document.getElementById("menuo").style.display = "none";
        
        document.getElementById("contactcall").style.display = "none";

        document.getElementById("contactadd").style.display = "none";

        document.getElementById("contactmail").style.display = "none";

        document.getElementById("social").style.display = "none";

        document.getElementById("mirror1").style.display = "none";
        }
        function link(){

            window.location = " https://www.linkedin.com/in/sumeet-gupta-ba2216209/";
        }
        
        function git(){
        
            window.location = "https://github.com/Sumeetbeginner";
        }
        
        function insta(){
        
            window.location = "https://www.instagram.com/sumeetguptaa/";
        }
        
        function mail(){
        
            window.location = "mailto: sumeetgupta3690@gmail.com";
        }
        